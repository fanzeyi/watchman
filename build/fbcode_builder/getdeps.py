...
class GenerateGitHubActionsCmd(ProjectCmdBase):
    def run_project_cmd(self, args, loader, manifest):
        ...
        if args.with_release:
            self.write_release_yaml(platforms, args)

    def _extract_platform_steps(self, platform, args):
        # This is really a hack since we don't have access to a proper yaml parser.
        platform_workflow = os.path.join(args.output_dir, f"getdeps_{platform}.yml")
        with open(platform_workflow, "r") as workflow:
            for line in workflow:
                if line.strip().startswith("build:"):
                    job = list(workflow)
                    # append release name to artifact name, so we don't mistakenly get artifacts from previous run
                    for ln in range(len(job) - 1, -1, -1):
                        if job[ln].rstrip().endswith(f"watchman-{platform}"):
                            job[ln] = (
                                job[ln].rstrip()
                                + "-${{ needs.prepare.outputs.release_name }}\n"
                            )
                    return "".join(job)
        print(
            f"WARN: unable to parse {platform_workflow} while trying to generate release workflow"
        )
        return ""

    def write_release_yaml(self, platforms, args):
        path = os.path.join(args.output_dir, f"getdeps_release.yml")
...
"""
            ...
            platform_jobs = []
            for p in platforms:
                (name, _) = platform_names[p.ostype]
                out.write(f"  {name}-build:\n")
                out.write("    continue-on-error: true\n")
                out.write("    needs: prepare\n")
                out.write(self._extract_platform_steps(name, args))
                platform_jobs.append(f"{name}-build")

            # prepare the release
            needs = ", ".join(platform_jobs)
            out.write(
                f"""
  release:
    needs: [prepare, {needs}]
...
"""
            ...
            needs = []
            for p in platforms:
                (platform, readable_name) = platform_names[p.ostype]
                needs.append(f"steps.{platform}-download.outcome == 'success'")
                out.write(
                    f"""
    - name: Download artifact for {readable_name}
      ...
      with:
        name: watchman-{platform}-${{{{ needs.prepare.outputs.release_name }}}}
"""
                )

            release_only = " || ".join(needs)
            out.write(
                f"""
    - name: Create release
      id: create_release
...
"""
            ...
            for p in platforms:
                (platform, readable_name) = platform_names[p.ostype]
                archive_name = (
                    "watchman-${{ needs.prepare.outputs.release_name }}-"
                    + p.ostype
                    + ".zip"
                )
                out.write(
                    f"""
    - name: Create archive for {readable_name}
      if: ${{{{  steps.{platform}-download.outcome == 'success' }}}}
      run: mv {platform} watchman-{platform} && zip -r {archive_name} watchman-{platform}/
    - name: Upload {readable_name} release
      if: ${{{{  steps.{platform}-download.outcome == 'success' }}}}
      env:
        GITHUB_TOKEN: ${{{{ secrets.GITHUB_TOKEN }}}}
...
"""
    ...
    def setup_project_cmd_parser(self, parser):
        ...
        parser.add_argument(
            "--with-release",
            action="store_true",
            help="Generate release.yml that helps you publish GitHub releases",
        )
